...
51  format(2F15.6)
    write(file_id, '(A30,2F15.6)') 'SITE POLARIZABLE ENERGY:', sp1*AUTOEV, sp2*AUTOEV
...
101 format(A30,2I10)
102 format(A30,F15.6)
103 format(A30,4F15.6)
    write(file_id,101) 'MO ID:', wf%i_mo, wf%j_mo
    write(file_id,102) 'MO OVERLAP:', wf%overlap
    write(file_id,103) 'Fock Matrix:', wf%parm * AUTOEV
    write(file_id,103) 'Fock Matrix with Lowdin:', wf%Heff * AUTOEV
